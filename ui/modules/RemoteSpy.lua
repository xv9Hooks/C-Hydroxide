...
-- MODERN İCONLAR VE UNRELİABLE EVENT ENTEGRASYONU
local icons = {
    ...
    RemoteEvent = "rbxassetid://136686644525835",
    UnreliableRemoteEvent = "rbxassetid://96897360575577",
    RemoteFunction = "rbxassetid://124676392956162",
    BindableEvent = "rbxassetid://92017078468380",
    BindableFunction = "rbxassetid://16126950699"
}
...
function Condition.new(remote, status, index, value, type)
    ...
    button:SetSelectedCallback(function() if not table.find(selected.conditions, condition) then table.insert(selected.conditions, condition) end end)
...
local function createConditions(remote)
    ...
    for index, arg in pairs(remote.IgnoredArgs) do
        for type in pairs(arg.types) do Condition.new(remote, "Ignore", index, nil, type) end
        for value in pairs(arg.values) do Condition.new(remote, "Ignore", index, value) end
    end
    for index, arg in pairs(remote.BlockedArgs) do
        for type in pairs(arg.types) do Condition.new(remote, "Block", index, nil, type) end
        for value in pairs(arg.values) do Condition.new(remote, "Block", index, value) end
    end
end
...
NewConditionButtons.Add.MouseButton1Click:Connect(function()
    ...
    elseif valueType == "Value" then
        ...
        elseif type == "boolean" then
            if value == "true" then value = true elseif value == "false" then value = false else return MessageBox.Show("Error", "Your input does not match the type you selected", MessageType.OK) end
        else
